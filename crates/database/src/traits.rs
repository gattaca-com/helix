...
pub trait DatabaseService: Send + Sync + Clone {
    async fn save_validator_registration(&self, entry: ValidatorRegistrationInfo, pool_name: Option<String>) -> Result<(), DatabaseError>;

    async fn save_validator_registrations(&self, entries: Vec<ValidatorRegistrationInfo>, pool_name: Option<String>) -> Result<(), DatabaseError>;
