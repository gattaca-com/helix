...
mod simulator_tests {
    ...
    fn get_optimistic_simulator(
        ...
        let db = MockDatabaseService::new(Arc::new(Default::default()), Arc::new(Default::default()));
    ...
    fn get_sim_req() -> BlockSimRequest {
        ...
        bid_trace.builder_public_key = BlsPublicKey::try_from(&get_test_pub_key_bytes(false)[..]).unwrap();
        ...
        let signed_bid_submission =
            SignedBidSubmission::Capella(SignedBidSubmissionCapella { message: bid_trace, execution_payload, signature: BlsSignature::default() });
    ...
    async fn test_process_request_optimistically_ok() {
        ...
        let simulator = get_optimistic_simulator(&server.url(), Some(builder_info.clone()), builder_demoted.clone());
