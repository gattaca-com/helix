...
pub struct RpcSimulator<DB: DatabaseService + 'static> {
    ...
    db: Arc<DB>,
    sim_method_v4: String,
    sim_method_v5: String,
}

impl<DB: DatabaseService + 'static> RpcSimulator<DB> {
    pub fn new(http: Client, simulator_config: SimulatorConfig, db: Arc<DB>) -> Self {
        let sim_method_v4 = format!("{}_validateBuilderSubmissionV4", simulator_config.namespace);
        let sim_method_v5 = format!("{}_validateBuilderSubmissionV5", simulator_config.namespace);
        Self { http, simulator_config, db, sim_method_v4, sim_method_v5 }
    }
