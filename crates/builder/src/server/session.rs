...
mod tests {
    ...
    fn version_negotiation() {
        let v = MERGING_PROTOCOL_VERSION;
        assert_eq!(negotiate_version(v, v), Some(v));
        assert_eq!(negotiate_version(v, v + 5), Some(v));
        assert_eq!(negotiate_version(v - 1, v), Some(v));
        assert_eq!(negotiate_version(v + 1, v + 5), None);
        assert_eq!(negotiate_version(0, v - 1), None);
    }
