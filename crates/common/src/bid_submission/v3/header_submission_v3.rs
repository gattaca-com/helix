...
mod tests {
    ...
    const V3_JSON: &str = include_str!("../testdata/header-submission-v3.json");
    const ELECTRA_JSON: &str = include_str!("../testdata/header-submission-electra.json");

    #[test]
    fn decode() {
        let v3: HeaderSubmissionV3 = serde_json::from_str(V3_JSON).unwrap();
        assert_eq!(4119478, v3.submission.bid_trace().slot);
    }

    #[test]
    fn decode_electra() {
        let sub: HeaderSubmissionElectra = serde_json::from_str(ELECTRA_JSON).unwrap();
