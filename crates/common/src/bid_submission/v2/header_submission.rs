...
mod tests {
    ...
    fn test_deserialise_signed_header_submission() {
        let v3_json = include_str!("../testdata/header-submission-v3.json");
        let json: Value = serde_json::from_str(v3_json).unwrap();
        let submission: SignedHeaderSubmission = serde_json::from_value(json["submission"].clone())
        ...
        assert!(result.is_ok(), "failed to decode SignedHeaderSubmission: {:?}", result);
        let decoded = result.unwrap();
        assert_eq!(decoded.as_ssz_bytes(), submission.as_ssz_bytes());
    }
}
