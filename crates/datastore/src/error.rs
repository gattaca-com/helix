...
impl IntoResponse for AuctioneerError {
    fn into_response(self) -> Response {
        match self {
            AuctioneerError::UnexpectedValueType => (StatusCode::BAD_REQUEST, "Unexpected value type".to_string()).into_response(),
            AuctioneerError::RedisError(err) => (StatusCode::INTERNAL_SERVER_ERROR, format!("Redis error: {err}")).into_response(),
            AuctioneerError::FromUtf8Error(err) => (StatusCode::BAD_REQUEST, format!("UTF-8 error: {err}")).into_response(),
            AuctioneerError::ParseIntError(err) => (StatusCode::BAD_REQUEST, format!("Parse Int error: {err}")).into_response(),
            AuctioneerError::PastSlotAlreadyDelivered => (StatusCode::BAD_REQUEST, "Past slot already delivered".to_string()).into_response(),
            AuctioneerError::AnotherPayloadAlreadyDeliveredForSlot => {
                (StatusCode::BAD_REQUEST, "Another payload already delivered for slot".to_string()).into_response()
            }
            AuctioneerError::SszDeserializeError(err) => (StatusCode::BAD_REQUEST, format!("SSZ deserialize error: {err}")).into_response(),
            AuctioneerError::SszSerializeError(err) => (StatusCode::BAD_REQUEST, format!("SSZ serialize error: {err}")).into_response(),
            AuctioneerError::ExecutionPayloadNotFound => {
                (StatusCode::BAD_REQUEST, "No execution payload for this request".to_string()).into_response()
            }
            AuctioneerError::BuilderNotFound { pub_key } => {
                (StatusCode::BAD_REQUEST, format!("Builder not found for public key: {pub_key:?}")).into_response()
            }
            AuctioneerError::EthereumConsensusError(err) => {
                (StatusCode::INTERNAL_SERVER_ERROR, format!("Ethereum consensus error: {err:?}")).into_response()
            }
            AuctioneerError::BroadcastStreamRecvError(err) => {
                (StatusCode::INTERNAL_SERVER_ERROR, format!("Broadcast stream recv error: {err}")).into_response()
            }
