...
impl BidSorter {
    ...
    /// Value of the bid that would be served for this fork right now.
    pub fn top_bid_value(&self, fork: &ForkKey) -> Option<U256> {
        self.forks.get(fork).and_then(|s| s.curr_bid.as_ref().map(|b| b.value))
    }
